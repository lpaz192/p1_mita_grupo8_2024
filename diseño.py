...
parte_superior_publicacion = lambda: print(f"┌{'─'*12}┬{'─'*20}┬{'─'*10}┬{'─'*15}┬{'─'*12}┬{'─'*20}┐")
encabezado_publicacion = lambda: print(f"│ {'ID':<10} │ {'Fecha':<18} │ {'Likes':<8} │ {'Comentarios':<13} │ {'ID Usuario':<10} │ {'Usuario':<18} │")
mostrar_publicacion = lambda id_post, fecha, likes, comentarios, id_usuario, usuario: print(f"│ {id_post:<10} │ {fecha:<18} │ {likes:<8} │ {comentarios:<13} │ {str(id_usuario).zfill(4):<10} │ {usuario:<18} │")
